...
client.on('messageCreate', async (message) => {
    // Check commands
    if (message.content.toUpperCase() === COMMANDS.init) {
    ...
    if (message.content.toUpperCase() == COMMANDS.help) {
        helpBoss();
        return;
    }

    if (message.content.toUpperCase() == COMMANDS.list || message.content.toUpperCase() == COMMANDS.boss) {
        getBoss();
        return;
    }

    if (message.content.toUpperCase() == COMMANDS.clearall) {
        clearAllBoss();
        return;
    }
...
const sendMessage = async (message) => {
  try {
    await client.channels.cache.get(sourceChannelID).send(message);
  } catch (err) {
